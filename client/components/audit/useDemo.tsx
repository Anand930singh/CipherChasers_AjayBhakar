...
function makeSeries(
  ...
  const colors = ["#e86418", "#653574"]; // Add more colors as needed
  const start = 0;
  ...
  return {
    label: `Series ${i + 1}`,
    color: colors[i % colors.length], // Assign color from the colors array based on index
    data: [...new Array(length)].map((_, i) => {
      ...
      const r = !useR
        ...
        : rMax -
          Math.floor(
            Math.log(Math.random() * (1.1 ** rMax - rMin) + rMin) /
              Math.log(1.1)
          );
